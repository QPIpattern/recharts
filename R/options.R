...
...){
    ...
    if (!is.null(itemStyle)) if (is.list(itemStyle))
        lstMark$itemStyle <- itemStyle
...
#' }
setSeries <- function(chart, timeslots=NULL, series=NULL, ...){
    if (!inherits(chart, 'echarts'))
        stop('chart is not an Echarts object. ',
             'Check if you have missed a %>% in your pipe chain.')
    validTypes <- c(
    ...
    if (is.null(timeslots)){  # no timeline
        if (length(series) >0)
            for (s in series){  # series is null, mono series
