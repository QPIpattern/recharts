...
series_map <- function(lst, type, subtype, return=NULL, ...){
    ...
    out <- lapply(lvlSeries, function(series){
        ...
        if (iSubtype != '')
            iSubtype <- validSubtypes[which(tolower(validSubtypes)==iSubtype)]
        if (iSubtype == "" || iSubtype == 'sum' || iSubtype == 'average'){
            mapType <- gsub("^.*(china|world|newmap).*$", "\\1", iType$misc)
        }else{
            if (mode=='series'){
                if (grepl('world', iType$misc))
                    mapType <-paste('world',
                          iSubtype[! iSubtype %in% c('sum', 'average')][1],
                          sep='|')
                else
                    mapType <- iSubtype[! iSubtype %in% c('sum', 'average')][1]
            }else{
                mapType <- paste(gsub("^.*(china|world).*$", "\\1", iType$misc),
                      iSubtype[! iSubtype %in% c('sum', 'average')][1], sep="|")
            }
        }
        o <- list(
            type=iType$type, roam=TRUE, mapType=mapType,
