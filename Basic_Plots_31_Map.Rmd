...
What if I don't know the coordinates of the places? You need `geocode` (forked from <github.com/caijun/geoChina>), which queries info from Google (so first you should be able to visit Google) or Baidu (in this way you need to provide api key). 

```{r}
coor <- geocode(c('北京', '上海'))
coor
```

These are coordinates under WGS-84 system. If you want to apply them in Echarts, you need to convert them to BD-09 system. You can use `convWGS2BD` function to do that.
...
target2 <- target
names(target2) <- c("group", 'name', 'value')
target2$series <- paste0(target2$group, 'Top10')  # series column mapping series of addML/addMP
...
Then append/modify the heatmap to a base map using `addHeatmap` / `overrideHeatmap`.
