...
echartr(grpmtx, Name, c(Group1, Group2, Group3, Group4), type='force') %>% 
...
```{r}
str(yuNetwork)
# nodes <- read.table(system.file('nodes.csv', package='recharts'), sep=',', 
#                     quote="\"", header=TRUE, fileEncoding='UTF-8', 
#                     stringsAsFactors=FALSE)
# links <- read.table(system.file('links.csv', package='recharts'), sep=',', 
#                     quote="\"", header=TRUE, fileEncoding='UTF-8',
#                     stringsAsFactors=FALSE)
# str(nodes)
# str(links)
...
# nodes <- cbind(nodes[,'name'], NA, nodes[,c('series', 'value')], 
#                stringsAsFactors=FALSE)
# links <- links[,c('source', 'target', 'relation', 'weight')]
# names(nodes) <- names(links)
# yu <- rbind(links, nodes, stringsAsFactors=FALSE)
# tmcn::getCharset()
# str(yu)
```
...
echartr(yu, c(source, target), weight, relation, type='force') %>% 
    setTitle("Yu Family of Shaoxing 绍兴俞氏") %>% setTheme(palette=c(
