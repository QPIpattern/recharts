...
- numeric `y`
- series` is used to produce separate trees
...
+--------+--------------------------------------------------------------------+
| series | series variable which will be coerced to factors. Each level of `series` is treated as a subsetting factor to produce separate trees. Only the first one is accepted if multiple variables are provided. |
+--------+--------------------------------------------------------------------+
...
- There are two series 'Tree 1' and 'Tree 2', so we will see two trees.
    - In the series 'Tree 1', we see 'North America' has NA parent node. So 'North America' will be the root node of this tree.
        - Below the root, we see 2 levels in parents nodes: 'USA' and 'Canada'. So 'North' tree will contain 2 level-1 nodes under the root 'North America'.
